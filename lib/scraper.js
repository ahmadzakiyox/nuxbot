...
async function igstalk(username) { 
    return new Promise(async(resolve, reject) => { 
        ...
        const { data } = await axios.get(`https://storiesdown.com/ajax.php?action=my_instagram_viewer_ajax&user_info=${username}&nonce=517c174617`, { 
            method: 'GET' 
        }); 
        const id = data.id
        const usernamee = data.username
        const fullname = data.full_name
        const biog = data.biography 
        const biolink = data.external_url
        const follower = data.edge_followed_by.count
        const following = data.edge_follow.count
        const bisnis = data.is_business_account 
        const profesional = data.is_professional_account 
        const privatee = data.is_private 
        const verified = data.is_verified 
        const profile = data.profile_pic_url_hd
        let result = { id, usernamee, fullname, biog, biolink, follower, following, bisnis, profesional, privatee, verified, profile }; 
        resolve(result);
       //console.log(result) 
       resolve(result)
        })
...
module.exports = { githubs, ssweb, igstalk, pinterest, wallpaper, wikimedia, quotesAnime, aiovideodl, umma, ringtone, styletext }
